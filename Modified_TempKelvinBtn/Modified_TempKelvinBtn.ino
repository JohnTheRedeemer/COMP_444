...
float degreesOther = 0;                         //the temperature in Fahrenheit, calculated from the voltage
String msgOther = "F";
...
void loop() {
  ...
  degreesOther = degreesC + 273.15; //convert the voltage to a temperature in degrees Fahrenheit
  ...
  if (digitalRead(3) == LOW) {
    msgOther = "K: ";
    degreesOther = degreesC + 273.15;
  } else {
    msgOther = "F: ";
    degreesOther = degreesC * (9.0 / 5.0) + 32.0;
  }

  lcd.setCursor(0, 1);                      //set the cursor to the lower left position
  lcd.print(msgOther);                 //Print a label for the data
  lcd.print(degreesOther);                      //print the degrees Fahrenheit
